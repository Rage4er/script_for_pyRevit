...
class SnakeForm(Form):
    def __init__(self):
        ...
        # Таймер
        self.timer = Timer()
        self.timer.Interval = 200  # Начальная скорость
    ...
    def update_status(self):
        if not self.game_started and not self.game_over:
            ...
            status = (
                "Игра окончена! Счет: {} | Скорость: {}x | ENTER - новая игра".format(
                    self.score, speed
                )
            )
        else:
            speed = int(200 / self.timer.Interval)
            status = "Змейка | Счет: {} | Скорость: {}x | Управление: ← → ↑ ↓".format(
                self.score, speed
            )
    ...
    def OnPaint(self, e):
        ...
        if not self.game_started and not self.game_over:
            ...
            g.DrawString(
                "Управление: ← → ↑ ↓", Font("Arial", 12), Brushes.Yellow, 120, 180
            )
            g.DrawString(
                "ENTER - начать игру", Font("Arial", 12), Brushes.LightGreen, 120, 200
            )
            g.DrawString(
                "N - новая игра", Font("Arial", 12), Brushes.LightBlue, 120, 220
            )
            ...
            g.DrawString(
                "ENTER - новая игра", Font("Arial", 12), Brushes.Yellow, 120, 220
            )
