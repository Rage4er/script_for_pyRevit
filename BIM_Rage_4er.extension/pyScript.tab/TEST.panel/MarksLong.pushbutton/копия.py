...
class MainForm(Form):
    ...
    def SetupTab4(self, tab):
        self.lstTagsOnView = ListView()
        self.lstTagsOnView.Location = Point(10, 40)
        self.lstTagsOnView.Size = Size(700, 400)
        self.lstTagsOnView.View = View.Details
        self.lstTagsOnView.GridLines = True
        self.lstTagsOnView.Columns.Add("Семейство", 250)
        self.lstTagsOnView.Columns.Add("Типоразмер", 250)
        self.lstTagsOnView.Columns.Add("Категория", 200)
    ...
    def OnDuplicateClick(self, sender, args):
        ...
        try:
            ...
            self.PopulateTagFamilies()  # Обновить список марок после дублирования
    ...
    def generate_new_name_and_num(self, tag_type):
        ...
        if len(parts) > 1:
            try:
                ...
                # Если после "_" не число, fallback к старому способу
        ...
        # Собрать существующие имена символов в рамках того же семейства
        family = tag_type.Family
        existing_names = set()
        symbol_ids = family.GetFamilySymbolIds()
        for symbol_id in symbol_ids:
            symbol = self.doc.GetElement(symbol_id)
            if symbol and symbol != tag_type:  # Исключить текущий символ
    ...
    def PopulateTagsOnView(self):
        self.lstTagsOnView.Items.Clear()
        view = self.uidoc.ActiveView
        if not view:
            return

        item = ListViewItem("Активный вид")
        item.SubItems.Add(view.Name if view else "Нет вида")
        item.SubItems.Add("")
        ...
        for element in collector:
            if element.Category and element.Category.Id.IntegerValue < 0:
                ...
                if builtin_cat in tag_categories:
                    ...
                    item = ListViewItem(family_name)
                    item.SubItems.Add(self.GetElementName(element))
                    category_name = (
        ...
        # Добавить общее количество марок в проекте для диагностики
        global_collector = FilteredElementCollector(self.doc).OfClass(FamilyInstance)
        global_count = 0
        for elem in global_collector:
            if elem.Category and elem.Category.Id.IntegerValue < 0:
                builtin_cat = BuiltInCategory(elem.Category.Id.IntegerValue)
                if builtin_cat in tag_categories:
                    global_count += 1

        view_count = self.lstTagsOnView.Items.Count
        item = ListViewItem("Всего марок в проекте")
        item.SubItems.Add(str(global_count))
        item.SubItems.Add("Марок на виде: " + str(view_count))
        self.lstTagsOnView.Items.Add(item)

        item = ListViewItem("Независимых марок на виде")
        item.SubItems.Add(str(independent_count))
        item.SubItems.Add("")
        self.lstTagsOnView.Items.Add(item)

        if self.lstTagsOnView.Items.Count == 0:
            item = ListViewItem("Марки не найдены")
            item.SubItems.Add("Активный вид не содержит марок")
            item.SubItems.Add("")
            self.lstTagsOnView.Items.Add(item)
    ...
    def GetElementName(self, element):
        ...
        if isinstance(element, FamilyInstance):
            ...
            if symbol:
                return self.GetElementName(symbol)
            else:
                return "Без типоразмера"
        try:
            if isinstance(element, FamilySymbol):
                ...
                type_name = ""
                for param_name in ["Тип", "Type Name"]:
                ...
                if type_name and type_name.startswith("IronPython"):
                    type_name = ""
                ...
                family_name = (
                    ...
                    and element.Family
                    and hasattr(element.Family, "Name")
                    and element.Family.Name
                    else ""
                )
