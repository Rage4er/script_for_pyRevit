...
__author__ = 'Rage'
__doc__ = 'Автоматическое размещение маркировочных меток на 3D-видах. Адаптивные марки с изменяемой длиной полки'
...
class TagSettings(object):
    def __init__(self):
        ...
        # НОВОЕ: Настройки адаптивных марок
        self.use_adaptive_tags = False
        self.category_parameters = {}  # {category: parameter_name}
...
# НОВЫЙ КЛАСС: Менеджер адаптивных марок
class AdaptiveTagManager:
    def __init__(self, doc):
        self.doc = doc
        self.adaptive_types_cache = {}
    
    def get_adaptive_tag_type(self, element, base_tag_type, param_name):
        """Получить адаптивный типоразмер марки с длиной полки на основе параметра"""
        if not param_name:
            return base_tag_type
        
        try:
            param_value = self.get_parameter_value_as_string(element, param_name)
            if not param_value:
                return base_tag_type
            
            shelf_length = self.calculate_shelf_length(param_value)
            adaptive_type = self.get_or_create_adaptive_type(base_tag_type, shelf_length)
            
            return adaptive_type if adaptive_type else base_tag_type
            
        except Exception as e:
            add_log("Ошибка создания адаптивной марки: " + str(e))
            return base_tag_type
    
    def get_parameter_value_as_string(self, element, param_name):
        """Получить строковое значение параметра элемента"""
        try:
    ...
    def calculate_shelf_length(self, param_value):
        """Рассчитать длину полки на основе значения параметра"""
        char_count = len(str(param_value))
        # Эмпирическая формула: базовая длина + длина текста
        shelf_length = char_count * 1.3 + 1.5
        return int(math.ceil(shelf_length))
    
    def get_or_create_adaptive_type(self, base_tag_type, shelf_length):
        """Получить или создать адаптивный типоразмер"""
        cache_key = str(base_tag_type.Id.IntegerValue) + "_" + str(shelf_length)
        
        if cache_key in self.adaptive_types_cache:
            return self.adaptive_types_cache[cache_key]
        
        existing_type = self.find_existing_adaptive_type(base_tag_type, shelf_length)
        if existing_type:
            self.adaptive_types_cache[cache_key] = existing_type
            return existing_type
        
        new_type = self.create_adaptive_type(base_tag_type, shelf_length)
        if new_type:
            self.adaptive_types_cache[cache_key] = new_type
            add_log("Создан адаптивный типоразмер: " + new_type.Name)
        
        return new_type if new_type else base_tag_type
    
    def find_existing_adaptive_type(self, base_tag_type, shelf_length):
        """Найти существующий адаптивный типоразмер"""
        try:
            base_name = base_tag_type.FamilyName
            expected_suffix = "_" + str(shelf_length)
            
            collector = FilteredElementCollector(self.doc)
            tag_types = collector.OfClass(FamilySymbol).WhereElementIsElementType().ToElements()
            
            for tag_type in tag_types:
                if (tag_type.FamilyName == base_name and 
                    tag_type.Name.endswith(expected_suffix)):
                    return tag_type
        except:
            pass
        return None
    
    def create_adaptive_type(self, base_tag_type, shelf_length):
        """Создать новый адаптивный типоразмер"""
        try:
            new_name = base_tag_type.Name + "_" + str(shelf_length)
            
            new_type = base_tag_type.Duplicate(new_name)
            
            # Устанавливаем длину полки (предполагаем что параметр называется "Длина полки")
            shelf_param = new_type.LookupParameter("Длина полки")
            if shelf_param and shelf_param.StorageType == StorageType.Double:
                shelf_param.Set(shelf_length / 304.8)  # Конвертируем мм в футы
            
            return new_type
            
        except Exception as e:
            add_log("Ошибка создания адаптивного типоразмера: " + str(e))
            return None

# НОВЫЙ КЛАСС: Форма выбора параметра категории (ВЫНЕСЕНА НА ВЕРХНИЙ УРОВЕНЬ)
class CategoryParamSelectionForm(Form):
    def __init__(self, category_name, available_params, current_param):
        self.category_name = category_name
        self.available_params = available_params
        self.selected_parameter = current_param
        
        self.InitializeComponent()
        self.PopulateParametersList()
    
    def InitializeComponent(self):
        self.Text = "Выбор параметра для категории: " + self.category_name
        ...
        self.lblParams = Label(Text="Выберите параметр для расчета длины полки:", 
                             Location=Point(10, 10), Size=Size(400, 20))
        
        self.lstParams = ListBox(Location=Point(10, 40), Size=Size(460, 300))
        
        self.btnOK = Button(Text="OK", Location=Point(300, 350), Size=Size(75, 25))
        self.btnCancel = Button(Text="Отмена", Location=Point(385, 350), Size=Size(75, 25))
        
        self.lstParams.DoubleClick += self.OnParamDoubleClick
        self.btnOK.Click += self.OnOKClick
        self.btnCancel.Click += self.OnCancelClick
        
        controls = [self.lblParams, self.lstParams, self.btnOK, self.btnCancel]
        for c in controls: 
            self.Controls.Add(c)
    
    def PopulateParametersList(self):
        self.lstParams.Items.Clear()
        for param_name in self.available_params:
            self.lstParams.Items.Add(param_name)
        
        # Выбираем текущий параметр если он есть
        if self.selected_parameter and self.selected_parameter in self.available_params:
            index = self.available_params.index(self.selected_parameter)
            self.lstParams.SelectedIndex = index
    
    def OnParamDoubleClick(self, sender, args):
        if self.lstParams.SelectedIndex >= 0:
...
class MainForm(Form):
    def __init__(self, doc, uidoc):
        ...
        # НОВОЕ: Менеджер адаптивных марок
        self.adaptive_manager = AdaptiveTagManager(doc)
    ...
    def InitializeComponent(self):
        ...
        # НОВОЕ: Добавляем вкладку для адаптивных настроек
        tabs = ["1. Выбор видов", "2. Категории", "3. Марки", "4. Настройки", "5. Адаптивные марки", "6. Выполнение"]
    ...
    # НОВАЯ ВКЛАДКА: Настройки адаптивных марок
    def SetupTab5(self, tab):
        self.lstCategoryParams = ListView()
        self.lstCategoryParams.Location = Point(10, 40)
        self.lstCategoryParams.Size = Size(700, 350)
        self.lstCategoryParams.View = View.Details
        self.lstCategoryParams.FullRowSelect = True
        self.lstCategoryParams.GridLines = True
        self.lstCategoryParams.Columns.Add("Категория", 200)
        self.lstCategoryParams.Columns.Add("Параметр для длины полки", 300)
        self.lstCategoryParams.Columns.Add("Доступные параметры", 200)
        self.lstCategoryParams.DoubleClick += self.OnCategoryParamDoubleClick
        
        self.chkUseAdaptiveTags = CheckBox()
        self.chkUseAdaptiveTags.Text = "Использовать адаптивные марки"
        self.chkUseAdaptiveTags.Location = Point(10, 400)
        self.chkUseAdaptiveTags.Size = Size(250, 20)
        self.chkUseAdaptiveTags.Checked = False
        
        controls = [
            self.CreateControl(Label, Text="Выберите параметры для расчета длины полки по категориям:", 
                             Location=Point(10, 10), Size=Size(500, 20)),
            self.lstCategoryParams,
            self.chkUseAdaptiveTags,
            self.CreateControl(Button, Text="← Назад", Location=Point(500, 450), Size=Size(80, 25)),
            self.CreateControl(Button, Text="Далее →", Location=Point(600, 450), Size=Size(80, 25))
        ]
        self.btnBack4, self.btnNext5 = controls[3], controls[4]
        self.btnBack4.Click += self.OnBack4Click
        self.btnNext5.Click += self.OnNext5Click
        for c in controls: 
            tab.Controls.Add(c)
    ...
    def SetupTab6(self, tab):
        ...
        self.btnBack5, self.btnExecute = controls[2], controls[3]
        self.btnBack5.Click += self.OnBack5Click
    ...
    def OnNext4Click(self, sender, args): 
        ...
        # НОВОЕ: Заполняем список категорий для выбора параметров
        self.PopulateCategoryParamsList()
        self.tabControl.SelectedIndex = 4
    
    def OnNext5Click(self, sender, args):
        # НОВОЕ: Сохраняем настройки адаптивных марок
        self.settings.use_adaptive_tags = self.chkUseAdaptiveTags.Checked
    ...
    # НОВЫЙ МЕТОД: Заполнение списка параметров категорий
    def PopulateCategoryParamsList(self):
        """Заполнение списка категорий и их параметров для адаптивных марок"""
        try:
            self.lstCategoryParams.Items.Clear()
            ...
            for category in self.settings.selected_categories:
                ...
                # Получаем доступные параметры для этой категории
                available_params = self.GetAvailableParametersForCategory(category)
                param_names = ", ".join(available_params[:3]) + ("..." if len(available_params) > 3 else "")
                
                # Устанавливаем текущий выбранный параметр (если есть)
                current_param = self.settings.category_parameters.get(category, "")
                
                item.SubItems.Add(current_param)
                item.SubItems.Add(param_names)
                
                self.lstCategoryParams.Items.Add(item)
                
        except Exception as e:
            add_log("PopulateCategoryParamsList error: " + str(e))
    
    # НОВЫЙ МЕТОД: Получение доступных параметров для категории
    def GetAvailableParametersForCategory(self, category):
        """Получение списка параметров для конкретной категории"""
        try:
            param_names = []
            
            # Получаем элементы этой категории для анализа параметров
            collector = FilteredElementCollector(self.doc)
            ...
            add_log("GetAvailableParametersForCategory error: " + str(e))
            return []
    
    # НОВЫЙ МЕТОД: Обработка двойного клика для выбора параметра
    def OnCategoryParamDoubleClick(self, sender, args):
        """Обработка двойного клика для выбора параметра категории"""
        if self.lstCategoryParams.SelectedItems.Count == 0:
            return
            
        selected_item = self.lstCategoryParams.SelectedItems[0]
        ...
        if available_params:
            form = CategoryParamSelectionForm(category_name, available_params, 
                                            self.settings.category_parameters.get(category, ""))
            if form.ShowDialog() == DialogResult.OK and form.SelectedParameter:
                # Сохраняем выбранный параметр для категории
                self.settings.category_parameters[category] = form.SelectedParameter
                selected_item.SubItems[1].Text = form.SelectedParameter
        else:
            MessageBox.Show("Нет доступных параметров для категории: " + category_name)
    ...
    def GenerateSummary(self):
        ...
        summary += "Адаптивные марки: " + ("Да" if self.settings.use_adaptive_tags else "Нет") + "\r\n"
        ...
        for category in self.settings.selected_categories:
            ...
            param_name = self.settings.category_parameters.get(category, "Не выбран")
            summary += "- " + self.GetCategoryName(category) + ": " + status + " (параметр: " + param_name + ")\r\n"
        return summary
    
    def OnExecuteClick(self, sender, args):
        add_log("Начало выполнения расстановки марок")
        success_count = 0
        errors = []
        ...
        try:
            for view in self.settings.selected_views:
                ...
                for category in self.settings.selected_categories:
                    ...
                    base_tag_type = self.settings.category_tag_types.get(category)
                    
                    if not base_tag_type:
                        errors.append("Нет марки для " + self.GetCategoryName(category))
                        continue
                    
                    for element in elements:
                        if element and not self.HasExistingTag(element, view):
                            # НОВОЕ: Выбор типоразмера марки
                            tag_type_to_use = base_tag_type
                            if self.settings.use_adaptive_tags:
                                param_name = self.settings.category_parameters.get(category)
                                adaptive_type = self.adaptive_manager.get_adaptive_tag_type(element, base_tag_type, param_name)
                                if adaptive_type:
                                    tag_type_to_use = adaptive_type
                                    add_log("Используется адаптивный типоразмер: " + self.GetElementName(adaptive_type))
                            
                            if self.CreateTag(element, view, tag_type_to_use):
                                success_count += 1
            
            trans.Commit()
            add_log("Создано марок: " + str(success_count))
            
        except Exception as e:
            trans.RollBack()
            errors.append("Ошибка выполнения: " + str(e))
        finally:
            trans.Dispose()
